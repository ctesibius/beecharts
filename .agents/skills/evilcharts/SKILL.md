...
description: >-
  ...
  sankey, heatmaps, histograms, pareto, gauges, or @evilcharts / BeeCharts
  components. Tells agents which Evil*Chart to install, which child parts to
  compose, when to use each chart type, and how to shape data. For contributing
  to the evilcharts repo itself, use evilcharts-build instead.
metadata:
  author: evilcharts
  version: "1.0.0"
---
...
BeeCharts is a **composable** chart library: one root (`Evil*Chart`) + shared UI + children you assemble (`<Bar />`, `<Grid />`, `<Tooltip />`, …). It is **not** a single `<Chart type="bar" />` API.
...
| [recipes.md](./recipes.md) | `@evilcharts/chart-recipes` — histogram, bullet, Pareto, heatmap, box plot, gauge |
| [examples.md](./examples.md) | Copy-paste reference implementations |

Repo docs: `https://beecharts.local/docs` (or local `/docs/<chart>`).
...
1. **Install chart + UI deps** — Every chart needs `@evilcharts/chart` at minimum; charts pull in `tooltip`, `legend`, `dot`, `background`, `bee-brush` as needed. See [install.md](./install.md).
...
| CLI | Role |
|-----|------|
| `@evilcharts/chart` | `ChartContainer`, `ChartConfig`, `LoadingIndicator` |
| `@evilcharts/tooltip` | `ChartTooltip` / `ChartTooltipContent` (used by chart `Tooltip`) |
| `@evilcharts/legend` | `ChartLegend` / `ChartLegendContent` |
| `@evilcharts/dot` | Point markers for line/scatter/radar |
| `@evilcharts/background` | `ChartBackground` / `backgroundVariant` on roots |
| `@evilcharts/bee-brush` | Zoom brush footer on bar/line/area/composed |
...
- **evilcharts-build** — only when modifying the evilcharts repository (registry, docs)
