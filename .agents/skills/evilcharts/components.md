...
```
<Evil*Chart config data …root props>
...
| `backgroundVariant` | Prefer `<ChartBackground variant="…" />` — use `graph-paper` for crosshatch (not `<Grid />`) |
| `showBrush` | Zoom range footer (bar, line, area, composed) |
| `defaultSelectedDataKey` / `onSelectionChange` | Legend/series selection |
| `chartProps` | Pass-through to Recharts chart |
...
| `barRadius`, `barGap`, `barCategoryGap` | Visual tuning |

**Children:** `Bar`, `XAxis`, `YAxis`, `Grid`, `Tooltip`, `Legend`

**Bar child:** `dataKey`, `variant` (hatched, gradient, …), `isClickable`, `barProps`
