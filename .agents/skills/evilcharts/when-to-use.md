...
Map user language → **one** `Evil*Chart` + optional **chart-recipes** helper.

Install helpers: `npx shadcn@latest add @evilcharts/chart-recipes` — see [recipes.md](./recipes.md).
...
| Heatmap | `BeeScatterChart` | `prepareHeatmapCells` → `ex-heatmap-chart` |
...
Different **data shaping**, **props** (`variant`, `layout`), or **Recharts escape hatches** (`barProps`, `scatterProps`) → extend `chart-recipes` + `ex-*` on the primitive doc.
