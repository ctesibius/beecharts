---
title: Funnel Chart
description: Stage-based funnel visuals built atop composable BeeCharts primitives
image: /og/funnel-chart.png
links:
  github: https://github.com/legions-developer/becocharts/blob/main/src/registry/charts/funnel-chart.tsx
...
<CodeTabs>
  ...
  <TabsPanel value="cli">
    <CliBlock commands={["@beecharts/funnel-chart"]} />
  </TabsPanel>
    ...
    <Steps>
      ...
      <Step>
        ...
        <StepContent>
          <ComponentSource
            name="funnel-chart"
            title="components/beecharts/charts/funnel-chart.tsx"
          />
        </StepContent>
      </Step>
       <Step>
...
The funnel chart is composible. `<BeeFunnelChart>` wraps a vertically laid out Recharts `<BarChart>` with custom trapezoid shapes per stage (`stageKey`) and measured values (`valueKey`). Compose `<Grid />`, `<XAxis />`, `<YAxis />`, `<Stages />`, `<Tooltip />`, `<Legend />` as needed, and set optional `backgroundVariant` on the chart root — Recharts has no standalone funnel primitive, but every native chart prop can pass through `chartProps` alongside the underlying <Link href="https://recharts.github.io/en-US/api/BarChart/" _blank>BarChart</Link> escape hatch documented below.
...
import {
  ...
  Tooltip,
  Legend,
} from "@/components/beecharts/charts/funnel-chart";
```
...
Funnel stages expand from the center on first paint (motion.dev — Recharts bar animation is off). Default `animationType="top-to-bottom"` staggers from the first stage downward. Also `"bottom-to-top"`, `"center-out"`, `"edges-in"`, or `"none"`. Honors reduce motion.

```tsx
<BeeFunnelChart animationType="center-out" ...>
```
...
<ApiHeading>BeeFunnelChart</ApiHeading>
...
<ApiTable>
  ...
  <ApiRow name="glowingStages" type="string[]" default="[]">
    Names matching `stageKey` values emitting glow filters akin to BeeBars.
  </ApiRow>
</ApiTable>

<ApiHeading>XAxis</ApiHeading>

Mirrors numeric scale even though BeeFunnel hides ticks by default to keep silhouette clean (`hide` forwarding Recharts semantics).

<ApiTable>
  <ApiRow name="hide" type="boolean" default="true">
    Default hides axis line / ticks consistent with BeeFunnels marketing aesthetic.
  </ApiRow>
