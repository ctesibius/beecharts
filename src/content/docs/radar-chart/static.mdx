...
links:
  github: https://github.com/legions-developer/becocharts/blob/main/src/registry/charts/radar-chart.tsx
...
<CodeTabs>
  ...
  <TabsPanel value="cli">
    <CliBlock commands={["@beecharts/radar-chart"]} />
  </TabsPanel>
    ...
    <Steps>
      ...
      <Step>
        ...
        <StepContent>
          <ComponentSource
            name="radar-chart"
            title="components/beecharts/charts/radar-chart.tsx"
          />
        </StepContent>
      </Step>
       <Step>
...
The radar chart is a composible compound component. `<BeeRadarChart />` is the root container — every visual part (`<PolarGrid />`, `<PolarAngleAxis />`, `<Tooltip />`, `<Legend />`, and the `<Radar />` series) is composed as a child, so you render exactly what you need.
...
import {
  ...
  Dot,
  ActiveDot,
} from "@/components/beecharts/charts/radar-chart";
```
...
<BeeRadarChart data={data} config={chartConfig}>
  <PolarGrid />
...
```tsx
<BeeRadarChart data={[]} config={chartConfig} isLoading>
  <PolarGrid />
  <PolarAngleAxis dataKey="skill" />
  <Legend />
  <Tooltip />
...
<ApiHeading>BeeRadarChart</ApiHeading>
