...
links:
  github: https://github.com/legions-developer/becocharts/blob/main/src/registry/charts/area-chart.tsx
...
<CodeTabs>
  ...
  <TabsPanel value="cli">
    <CliBlock commands={["@beecharts/area-chart"]} />
  </TabsPanel>
    ...
    <Steps>
      ...
      <Step>
        ...
        <StepContent>
          <ComponentSource
            name="area-chart"
            title="components/beecharts/charts/area-chart.tsx"
          />
        </StepContent>
      </Step>
       <Step>
...
import {
  ...
  Dot,
  ActiveDot,
} from "@/components/beecharts/charts/area-chart";
```

```tsx
<BeeAreaChart data={data} config={chartConfig} stackType="stacked">
  <Grid />
  <XAxis dataKey="month" />
  <YAxis />
  <Legend isClickable />
...
Add `isClickable` to any `<Area>` (and to `<Legend>`) to make those series selectable. Use the `onSelectionChange` callback on `<BeeAreaChart>` to handle selection events:
...
<ApiHeading>BeeAreaChart</ApiHeading>
