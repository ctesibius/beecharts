...
links:
  github: https://github.com/legions-developer/becocharts/blob/main/src/registry/charts/composed-chart.tsx
...
<CodeTabs>
  ...
  <TabsPanel value="cli">
    <CliBlock commands={["@beecharts/composed-chart"]} />
  </TabsPanel>
    ...
    <Steps>
      ...
      <Step>
        ...
        <StepContent>
          <ComponentSource
            name="composed-chart"
            title="components/beecharts/charts/composed-chart.tsx"
          />
        </StepContent>
      </Step>
       <Step>
       ...
       <Step>
        ...
        <StepDescription>
          Finally, create a file called `dot.tsx` inside the `beecharts/ui` folder and paste the code there.
        </StepDescription>
        <StepContent>
          <ComponentSource
            name="dot"
...
import {
  ...
  Tooltip,
  Legend,
} from "@/components/beecharts/charts/composed-chart";
```
...
The basic preview matches [bar](/docs/bar-chart) and [line](/docs/line-chart): twelve months, `desktop` bars, `mobile` line, same emerald/rose tokens, and the same default **8px** top corner radius on bars (`barRadius` / `DEFAULT_BAR_RADIUS`).
...
Use `prepareParetoData()` from [Chart recipes](/docs/chart-recipes), then bar + line on `<BeeComposedChart />` with left/right Y axes. Pareto uses its own category data (not the traffic dataset above).
...
<ApiHeading>BeeComposedChart</ApiHeading>
