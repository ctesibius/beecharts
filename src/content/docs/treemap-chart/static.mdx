...
links:
  github: https://github.com/legions-developer/becocharts/blob/main/src/registry/charts/treemap-chart.tsx
...
<CodeTabs>
  ...
  <TabsPanel value="cli">
    <CliBlock commands={["@beecharts/treemap-chart"]} />
  </TabsPanel>
    ...
    <Steps>
      ...
      <Step>
        ...
        <StepContent>
          <ComponentSource
            name="treemap-chart"
            title="components/beecharts/charts/treemap-chart.tsx"
          />
        </StepContent>
      </Step>
       <Step>
...
import {
  ...
  Tooltip,
  Legend,
} from "@/components/beecharts/charts/treemap-chart";
```

```tsx
<BeeTreemapChart config={chartConfig} data={treemapData} backgroundVariant="dots">
...
Add `isClickable` on `<Tiles />` (and on `<Legend />`) so clicks select a tile. Use `onSelectionChange` on `<BeeTreemapChart>` — it receives `{ dataKey, value } | null`, where `dataKey` is the tile name from `nameKey` and `value` comes from `dataKey` (defaults to cumulative `size` for nested nodes):
...
<ApiHeading>BeeTreemapChart</ApiHeading>
...
<ApiTable>
  ...
  <ApiRow name="treemapProps" type='Omit<TreemapProps, "data" | "dataKey">'>
    Additional props forwarded to the underlying Recharts Treemap component (`data`, `dataKey`, `content`, gradients, strokes, naming keys, and sizing already owned by BeeTreemap internals). Read the <Link href="https://recharts.github.io/en-US/api/Treemap/" _blank>Recharts Treemap documentation</Link> for available escape hatches.
  </ApiRow>
</ApiTable>

<ApiHeading>Tiles</ApiHeading>

A configuration slot consumed by `<BeeTreemapChart />` — renders nothing alone. Toggle click behavior, glowing filters, and in-tile labels for every rectangle.
