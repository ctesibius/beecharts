...
links:
  github: https://github.com/legions-developer/becocharts/blob/main/src/registry/charts/bar-chart.tsx
...
<CodeTabs>
  ...
  <TabsPanel value="cli">
    <CliBlock commands={["@beecharts/bar-chart"]} />
  </TabsPanel>
    ...
    <Steps>
      ...
      <Step>
        ...
        <StepContent>
          <ComponentSource
            name="bar-chart"
            title="components/beecharts/charts/bar-chart.tsx"
          />
        </StepContent>
      </Step>
       <Step>
...
Bar, line, and composed **basic** previews share the same twelve-month `desktop` / `mobile` dataset, color tokens, and **8px** rounded bar tops (`barRadius` on the chart root).
...
Bin numeric samples with `binForHistogram()` from [Chart recipes](/docs/chart-recipes), then render with `variant="histogram"` on `<BeeBarChart />` (touching bins, square corners).
...
<ApiHeading>BeeBarChart</ApiHeading>
