...
links:
  github: https://github.com/legions-developer/becocharts/blob/main/src/registry/charts/waterfall-chart.tsx
      ...
      <Step>
        ...
        <StepContent>
          <ComponentSource
            ...
            title="components/beecharts/charts/waterfall-chart.tsx"
          />
        </StepContent>
      </Step>
       <Step>
...
import {
  ...
  Tooltip,
  Legend,
} from "@/components/beecharts/charts/waterfall-chart";
```
...
Bars grow in on first paint with a staggered motion.dev reveal (Recharts’ built-in bar animation is disabled, same as other Evil charts). Control order with `animationType` on the root — `"left-to-right"` (default), `"right-to-left"`, `"center-out"`, `"edges-in"`, or `"none"`. Respects the OS reduce-motion preference.

```tsx
<BeeWaterfallChart animationType="center-out" ...>
```
...
<ApiHeading>BeeWaterfallChart</ApiHeading>
...
<ApiTable>
  ...
  <ApiRow name="animationType" type='"none" | "left-to-right" | "right-to-left" | "center-out" | "edges-in"' default='"left-to-right"'>
    Staggered grow-in order on first render (motion.dev, not Recharts).
  </ApiRow>
...
<ApiTable>
  <ApiRow name="WaterfallType" type='"start" | "increase" | "decrease" | "total"'>
    Determines how `<BeeWaterfallChart />` maps each row onto transparent base offsets vs visible segments — `total` resets running sums to anchored bars, decreases invert direction while preserving stacked geometry, increases extend running totals, `start` seeds initial balances from absolute magnitudes.
  </ApiRow>
</ApiTable>
