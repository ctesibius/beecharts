...
export function resolveFunnelLayout(
  ...
  const connection = style?.connection ?? ctx.funnelConnection ?? "default";
  const taper = style?.taper ?? ctx.funnelTaper ?? "default";
  const preset = FUNNEL_CONNECTION[connection];
  const gap = style?.stageGap ?? ctx.stageGap ?? preset.gap;
