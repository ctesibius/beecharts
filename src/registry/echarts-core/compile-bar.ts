...
function buildBarDataPoints(
  ...
  const r = isHistogram
    ? resolveBarRadius(bar.radius, ctx.barRadius ?? 0)
    : resolveBarRadius(bar.radius, ctx.barRadius);
...
export function compileBarOption(ctx: CompileContext): EChartsOption {
  ...
  const stack =
    ctx.stackType === "stacked" || ctx.stackType === "percent" ? "bee-stack" : undefined;
  const barKeys = bars.map((bar) => bar.dataKey);
  const rows =
    ctx.stackType === "percent" ? normalizeStackPercent(ctx.data, barKeys) : ctx.data;
  ...
  const valueAxis = {
    ...
    max: ctx.stackType === "percent" ? 100 : undefined,
    axisLabel: ctx.stackType === "percent" ? { formatter: "{value}%" } : undefined,
  };

  const hasMonospace = bars.some((bar) => bar.variant === "monospace");
  const hasHoverTrace = bars.some((bar) => bar.variant === "hover-trace");
  const isHistogram = ctx.variant === "histogram";
