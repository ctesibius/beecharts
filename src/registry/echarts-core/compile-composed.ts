...
export function compileComposedOption(ctx: CompileContext): EChartsOption {
  ...
  const barSeries = bars.map((bar) => {
    const r = resolveBarRadius(bar.radius, ctx.barRadius);
  ...
  const grid = resolveCartesianGrid(ctx.parts, ctx.externalBrush, gridBottomWithZoom(hasBrush));
  if (yAxes.some((y) => y.orientation === "right") && !ctx.externalBrush) {
    grid.right = 56;
  }
