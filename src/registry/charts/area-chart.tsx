...
function AreaChartCanvas<TData extends Record<string, unknown>>({
  data,
  xDataKey,
  stackType,
  externalBrush,
  ...
  const { option, colorEpoch } = useCompiledOption(compileAreaOption, {
    data,
    xDataKey,
    stackType,
    externalBrush,
...
>({
  ...
  showBrush = true,
  brushFormatLabel,
}: BeeAreaChartProps<TData, TConfig>) {
        ...
        footer={
          showBrush && !isLoading ? (
            <BeeChartBrush
              ...
              rootFields={{ xDataKey: xKey, stackType, externalBrush: true }}
              xDataKey={xKey}
